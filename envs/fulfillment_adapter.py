...
class FulfillmentAdapter:
    ...
    def __init__(
        ...
        self.low_batt_threshold = 0.15
        self.low_batt_penalty = 0.5
    ...
    def step(
        ...
        for agent_id in range(num_agents):
            ...
            if dist_to_goal < 0.5 and not state.get('at_goal', False):
                task_completed = True
                state['at_goal'] = True
            ...
            if self.strict_paper_mode:
                ...
                r_batt = remain_battery
                if state['at_charger']:
                    r_batt += 0.5
                if remain_battery < self.low_batt_threshold:
            ...
            raw_R_step.append(r_main)
            raw_time_step.append(r_time)
            raw_batt_step.append(r_batt)
        ...
        # 归一化回报（同向化：越大越好）
        raw_R_arr = np.array(raw_R_step, dtype=np.float32)
        raw_time_arr = np.array(raw_time_step, dtype=np.float32)
        raw_batt_arr = np.array(raw_batt_step, dtype=np.float32)
        ...
        info_dict = {
            ...
            'raw_R': raw_R_arr.astype(np.float32).tolist(),
            'raw_time': raw_time_arr.astype(np.float32).tolist(),
            'raw_batt': raw_batt_arr.astype(np.float32).tolist(),
            'completed_tasks_this_step': float(completed_this_step),
        }
